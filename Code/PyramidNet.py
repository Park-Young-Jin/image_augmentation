...
class PyramidNet(nn.Module):
    ...
    def __init__(self, dataset, depth, alpha, num_classes, bottleneck=False):
        ...
        if self.dataset.startswith('cifar') or dataset == 'imagenet32':
            self.inplanes = 16
            if bottleneck == True:
                n = int((depth - 2) / 9)
                block = Bottleneck
            ...
            self.input_featuremap_dim = self.inplanes
            self.conv1 = nn.Conv2d(3, self.input_featuremap_dim, kernel_size=4, stride=2, padding=1, bias=False)
            self.bn1 = nn.BatchNorm2d(self.input_featuremap_dim)
    ...
    def forward(self, x):
        if self.dataset == 'cifar10' or self.dataset == 'cifar100' or self.dataset == 'imagenet32' or self.dataset == 'cifar10_subset' :
            x = self.conv1(x)
            x = self.bn1(x)
